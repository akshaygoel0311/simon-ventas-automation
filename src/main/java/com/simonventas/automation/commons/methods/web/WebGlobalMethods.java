package com.simonventas.automation.commons.methods.web;

import com.simonventas.automation.commons.helpers.driver.web.DriverWebBase;
import com.simonventas.automation.commons.utils.JsonParser;
...
// fin metodos scroll
    ...
    public void  clickSubmitElement(String element){
        getEntity(element).submit();
    }


    public void selectElementDropDown(String elementDropDown, String selectElement){
       WebElement element = getEntity(elementDropDown);
       boolean isVisible = visibleElement(elementDropDown,10);
       if(isVisible){
           Select companies = new Select(element);
           companies.selectByVisibleText(selectElement);
       }
    }
    
    public void selectElementDropDownbyValue(String elementDropDown, String selectElement){
        WebElement element = getEntity(elementDropDown);
        boolean isVisible = visibleElement(elementDropDown,10);
        if(isVisible){
            Select companies = new Select(element);
            companies.selectByValue(selectElement);
        }
     }
